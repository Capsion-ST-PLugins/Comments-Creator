...
class ParamsParser():
    ...
    def match_line(self, line_str:str) -> bool:
        ...
        if 'checkType' in self.reg_obj:
            ...
            # 获取缩进和其他参数
            self.result["line_start_indent"] = self.line_start_indent(result[reg['indent']])
            ...
            for each_reg in self.reg_obj['func']:
                ...
                # 获取缩进和其他参数
                self.result["line_start_indent"] = self.line_start_indent(result[each_reg['indent']])
                self.result["type"] = result[each_reg['type']] if 'type' in each_reg else ""
    ...
    def format(self, syntax_tmpl:dict=None):
        ...
        if not self.syntax_tmpl:return self

        # 处理 description 行
        description = self.get_description(self.syntax_tmpl['comments_contexts']['Description'])

        # 处理 returns 行
        returns = self.syntax_tmpl['comments_contexts']['returns'] + '\n'
        if 'return' in self.result:
            returns = returns.format(type='{'+self.result['return']['type']+'}', description=self.extract_description('returns'))
    ...
    def get_params(self, params_str:str, params_key_list:list) -> list:
        ...
        ```
        """
        if not params_str or len(params_str) == 0: return []
        ...
        res_list = [res for each in params_key_list for res in self.extract_params(each, params_list) if len(res)>0]
        ...
        for index,each_param in enumerate(params_list):
            ...
            for each_reg in self.reg_obj["paramType"][tar]:
                ...
                if 'context' in each_reg:
                    context = '{}={}'.format(res[0][each_reg['name']], res[0][each_reg['context']])
                else:
