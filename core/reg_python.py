...
float_reg = [
    create_reg_obj(r'^(\w+)\s*?\=\s*?([1-9]\d*\.\d*|0\.\d*[1-9]\d*$|-([1-9]\d*\.\d*|0\.\d*[1-9]\d*)$)', 3, 0, context=2),
    create_reg_obj(r'^(\w+)\s*?:\s*?(float)\s*?\=\s*?([1-9]\d*\.\d*|0\.\d*[1-9]\d*$|-([1-9]\d*\.\d*|0\.\d*[1-9]\d*)$)', 4, 0, 1, 2),
...
func_reg = [{
    ...
    "reg":re.compile(r'^(\s*)?(def)\s+(\w+)\(\s*?(.*)\s*?\)\s*?(\->)?\s*?([a-zA-Z\.->\s\[\]\$]+)?\s*?\:'),
    "res_len":6,
    "name":2,
    "type":1,
    "return_type":5,
...
class_reg = [{
    "reg":re.compile(r'^(\s*)?class\s*?(\w+)?\((.*)\)\s*?(\->)?\s*?(\w+\.\w+|\w+)?\s*?\:'),
    "res_len":5,
    "name":2,
    "type":1,
    "return_type":5,
