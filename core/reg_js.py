...
func_reg = [
    {  # export const ifDirExists = async (filePath="") => { 处理有括号的
        "reg": re.compile(
            r"^(\s+)?(export\s)?(const\s|var\s|let\s)(\w+\s?)\=\s?(async\s?)?\((.*)\)\s?\:?(.*)?\s?\=>\s?\{?"
        ),
        "res_len": 7,
        "name": 3,  # 函数名
        "return_type": 6,  # 返回的类型
    ...
    {
        "reg": re.compile(
            r"^(\s+)?(exports\.)(\w+)\s?\=\s?(async\s)?(function)\s?\((.*)?\)\s?\:?\s?(\w+)?\s?\{"
        ),
        "res_len": 7,
        "name": 2,
        "return_type": 6,
    ...
    {
        "reg": re.compile(
            r"^(\s+|exports\.|\w+\.)(\w+\s?)\=\s?(async\s?)?\((.*)\)\s?(\:.*)?\s?\=>\s?\{?"
        ),
        "res_len": 5,
        "name": 1,
        "type": 1,
            ...
            result.append(
                {
                    ...
                    "context": res[0][each_reg["context"]]
